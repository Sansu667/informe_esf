...
# Comparación BLG vs Global en Eficiencia
...
ggplot(ci_data, aes(x = Metric, y = Mean_Global)) +
  ...
  theme_minimal() +
  theme(axis.text.x = element_blank()) + # Eliminar texto redundante del eje X
...
# Datos para gráficos con ggplot2
...
# Renombrar columnas para evitar guiones bajos (que rompen LaTeX con escape=FALSE)
colnames(df_anova) <- c("Fuente", "Df", "Sum Sq", "Mean Sq", "F Value", "P Value")

# Corregir el símbolo < para LaTeX (evitar que salga como ¡)
